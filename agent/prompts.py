...
BRAIN_ORCHESTRATION_GUIDE = (
    "## 多智能体任务编排\n"
    "**仅当任务足够复杂（需要多步骤、多数据源、大量分析工作）时才使用编排**。\n\n"
    "**以下情况直接执行，禁止编排**：\n"
    "- 简单的信息搜索（打开浏览器搜XX、查XX数据）→ 自己用 browser/search 工具直接完成\n"
    "- 打开/读取文件 → 直接用 read_file\n"
    "- 简单问答 → 直接回答\n"
    "- 任何单一工具调用就能完成的任务\n\n"
    "**以下情况可使用编排**：\n"
    "- 需要同时从多个数据源收集大量信息\n"
    ...
    "**严禁以下行为（节省轮次，快速交付）：**\n"
    "- **禁止在创建计划前用 `send_to_agent` 打探 worker 的能力**。"
    "create_task_plan 会自动获取在线智能体列表并匹配角色，"
    "你只需要直接调用它，不要先和 worker 聊天。\n"
    "- **禁止在 worker 工作时催促或询问进度**。"
    "系统会自动更新状态，用 `check_plan_progress` 仅在用户主动询问进度时调用。\n"
    "- worker 交付后直接汇总，不要再让 worker \"确认\"或\"补充\"——除非产出明显不符合需求。"
)
# 五、对话终止判断提示词
# ============================================================
...
CLARIFICATION_PROMPT = """你是一个需求分析师。以下是近期对话上下文，帮助你理解用户当前消息所指的背景：
...
请结合对话上下文判断用户目标是否足够清晰。

**关键原则：分清「任务」和「信息查询」**：
- **信息查询类（直接放行）**：用户只是问"XX是什么""查询XX数据""搜索XX""XX的最新消息""XX票房"等，这些是简单的信息检索任务，**不需要澄清**，直接搜索即可。用你脑中不确定的知识去质疑用户的描述（如"没有这部片名"）是不可接受的——你应该先搜索确认，而不是先追问用户。
- **任务类（需要时才澄清）**：用户提出开发、设计、写作、操作等复杂任务，若缺乏关键信息（目标用户、核心功能、验收标准等），才需要澄清。

**追问约束**：
- 用户重复原始请求而不直接回答你的问题时，说明用户在拒绝追问 → 必须判定为 clear=true。
- 同一轮中你问过的问题不要重复追问。如果历史记录显示类似问题已被问过 → 必须判定为 clear=true。
- 信息查询类任务一律判定 clear=true，澄清的数目不超过一轮。

若不够清晰且符合追问条件，请提出1-2个最需要澄清的问题。
若已足够清晰，请将原始需求与澄清内容整合为一段完整的目标描述。
...
**并行优先原则**：当有多种数据源或分析维度时，应创建可并行的子任务
（分别分配给不同的 worker），而非一个串行子任务。例如：
- "收集 LLM 框架资料" 和 "收集 Agent 框架资料" 可并行派发给两个搜索专家
- "分析竞品 A" 和 "分析竞品 B" 可并行派发给两个数据分析师
- 仅当后一个任务确实需要前一个任务的输出时，才建立依赖关系

**重要：你是 orchestrator（编排者），你只委派数据收集、分析、搜索等具体执行工作给 worker。**
**最终向用户呈现报告/汇总/结论，是你自己的职责，不要创建"撰写最终报告"或"向用户汇报"之类的子任务。**
**worker 交付分析结果后，系统会自动通知你整合并向用户呈现，所以子任务只需覆盖到"分析和产出"即可。**
...
4. suggested_role 用于匹配合适的在线智能体。**关键规则：如果有多个在线智能体可用，你必须在子任务之间分配不同的 suggested_role，使工作均匀分布到不同的智能体上。禁止所有子任务使用相同或相似的角色名。**
...
8. **角色分配指引**：
   - 含"搜索""收集""检索""查找"关键词的任务 → suggested_role 用"搜索专家"
   - 含"分析""整理""归纳""总结"关键词的任务 → suggested_role 用"数据分析师"
   - 两个角色必须在子任务中各出现至少一次（如果在线智能体中同时存在搜索和分析类智能体）
