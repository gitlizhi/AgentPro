...
class Brain:
    ...
    async def _generate_thought(self, user_id: str = "super_user") -> str:
        """生成一个随机想法，结合记忆和最近对话"""
        thought_types = [
            "基于用户的长期记忆，给用户一个生活建议或提醒。",
            "回顾最近与用户的对话，提出一个跟进问题或有趣的观点。",
            "根据当前时间和用户的习惯，想一个贴心的提醒。",
            "从用户的记忆中找一个有趣的事实，然后以此为主题闲聊。",
    ...
    async def _think_and_act(self):
        ...
        if self.comm:
            await self.comm.send_to_agent(target_user, {"text": f" {thought}"})
    ...
    async def _get_recent_messages(self, user_id: str, limit: int = 5) -> list:
        ...
        try:
            state = await self.checkpointer.aget_tuple(config)  # 注意方法名可能是 aget_tuple
            if not state:
                return []
            # 从 checkpoint 中提取消息
